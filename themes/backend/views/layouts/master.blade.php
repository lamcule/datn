...
<script>
  window.MonCMS = {
    ...
    user_token: '{{ $currentUser->getFirstTokenKey() }}',
    multipleLanguage: '{{config('mon.multiple_languages')}}',
    permissions: {!! json_encode($permissions) !!},
    permissionDenied: '{{trans('backend::mon.message.permission_denied')}}',
    categories: {!! json_encode(config('student.categories')) !!},
    personal_categories: {!! json_encode(config('student.personal_categories')) !!}

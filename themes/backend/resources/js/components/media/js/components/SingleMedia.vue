<template>
    <div>
        ...
        <div class="jsThumbnailImageWrapper jsSingleThumbnailWrapper" v-if="hasSelectedMedia">
            ...
            <el-button type="button" @click="unSelectMedia">{{ $t('media.remove_media') }}</el-button>
        </div>
        <div class="" v-else>
        ...
        <el-dialog
            ...
            <span slot="footer" class="dialog-footer">
                <el-button @click="dialogVisible = false">{{ $t('mon.button.cancel') }}</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            openImage() {
                window.open(this.selectedMedia.full_url, "_blank");
            },
            handleClose(done) {
                done();
            },

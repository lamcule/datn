...
class EloquentStudentRepository extends BaseRepository implements StudentRepository
{
    ...
    {
        $query = $this->getStudentLessonReportQuery($request);
        return $query->paginate($request->get('per_page', 25));
    }

    public function getStudentLessonReportQuery(Request $request)
    {
        $lesson_id = $request->get('lesson_id');
        $query = $this->newQueryBuilder();

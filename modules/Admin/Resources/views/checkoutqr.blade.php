    ...
    margin-bottom: 20px;">
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td><span class="badge {{$lesson->getQRTypeClass(trans('backend::lesson.web.review'))}}">{{trans('backend::lesson.web.checkout')}}</td>
                    </tr>
                    <tr>
                        <th style="width:30%">{{trans('backend::lesson.web.type')}}:</th>
                        <td>{{trans('backend::lesson.web.checkout')}}</td>
                    </tr>
                    </tbody>
                </table>
            </div>
            {!! QrCode::size(480)->generate(route('checkout', ['lesson' => $lesson->id], true)); !!}
        </div>
    </div>
@endsection
@push('css-stack')

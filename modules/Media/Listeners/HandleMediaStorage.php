...
class HandleMediaStorage
{
    ...
    private function handleMultiMedia(StoringMedia $event)
    {
        ...
        foreach ($postMedias as $zone => $attributes) {
            ...
            foreach (array_get($attributes, 'files', []) as $key=> $fileId) {
                $syncList[$fileId] = [];
                $syncList[$fileId]['mediable_type'] = get_class($entity);
                $syncList[$fileId]['zone'] = $zone;
                $syncList[$fileId]['order'] = (int)$key;
    ...
    private function handleSingleMedia(StoringMedia $event)
    {
        ...
        foreach ($postMedia as $zone => $fileId) {
            if (!empty($fileId)) {
                $entity->filesByZone($zone)->sync([$fileId => ['mediable_type' => get_class($entity), 'zone' => $zone, 'order' => null]]);
            } else {
